...
/**
 * Generate cross-stitch patterns for an image, by pixelating the image and then
 * mapping each super-pixel in the image to a floss color by calculating the
 * "closest" color of available floss.
 */
public class Pattern extends ImageControl {

	private List<int[]> floss_list = new ArrayList<>();
	private List<String> symbol_list = new ArrayList<>();
	...
	public void pattern(int[][][] image, int square, String saveName) {
		if (image == null || saveName == null || square == 0) {
		...
		try {
			...
			for (int i = 0; i < used_floss_list.size(); i++) {
				...
				myWriter.write(symbol_list.get(index) + "  " + name_list.get(index) + System.lineSeparator());
	...
	public int[][][] stichPattern(int[][][] image, int square, int[][][] pixelate) {
		try {
